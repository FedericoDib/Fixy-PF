...
export default function UseGeolocation() {
    ...
    useEffect(() => {
        (async () => {
            ...
            setLocation({ latitude: location.coords.latitude, longitude: location.coords.longitude, latitudeDelta, longitudeDelta });
        })();
    }, []);
    ...
    if (location !== null) {
        ...
        fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + location.latitude + "," + location.longitude + "&key=" + "AIzaSyCjaUjNivDjo1UMsuiLljUIEZrwHooJgxU").then((response) => response.json()).then((responseJson) => {
            setAddress(
